...
from astrodash.helpers import calc_params_for_log_redshifting

#from dash_cnn_v2_test import build_model, evaluate_model
from dash_cnn_model import AstroDASH, HyperParameterTuning
...
def train_model(dataDirName, overwrite=False, numTrainBatches=500000, minZ=0., maxZ=0., redshifting=False):
    ...
    dataset = os.path.join(dataDirName, 'dataset.zip')
    extractedFolder = os.path.join(dataDirName, 'dataset_set')
    ...
    if os.path.exists(extractedFolder) and overwrite:
        ...
        os.system("unzip %s -d %s" % (dataset, extractedFolder))
    ...
    # print(npyFiles)
    trainImages = np.load(npyFiles['trainImages'], allow_pickle=True)
    trainLabels = np.load(npyFiles['trainLabels'], allow_pickle=True)
    testImages = np.load(npyFiles['testImages'], allow_pickle=True)
    testLabelsIndexes = np.load(npyFiles['testLabels'], allow_pickle=True)
    typeNamesList = np.load(npyFiles['typeNamesList'], allow_pickle=True)
    testTypeNames = np.load(npyFiles['testTypeNames'], allow_pickle=True)
    ...
    snTypes = ['Ia-norm', 'Ia-91T', 'Ia-91bg', 'Ia-csm', 'Iax', 'Ia-pec',
               'Ib-norm', 'Ibn', 'IIb', 'Ib-pec', 'Ic-norm', 'Ic-broad',
               'Ic-pec', 'IIP', 'IIL', 'IIn', 'II-pec','TDE','KN','SLSNe']
    ...
    #print(nLabels)
    # dash = HyperParameterTuning(trainImages, trainLabels, testImages, testLabelsIndexes, N, nLabels, typeNamesList,
    #                  testTypeNames, snTypes)
    # pred_Labels=dash.perform_tuning()
    # val, freq = np.unique(trainLabels, return_counts=True)
    # print(val, "\n\n", freq, "\n\n", len(trainLabels), sum(freq))
    # weights = freq/sum(freq)
    # print(weights)
    dash = AstroDASH(trainImages, trainLabels, testImages, testLabelsIndexes,
                     N, nLabels, typeNamesList, testTypeNames, snTypes)
    dash.build_model()
    #dash.cross_validation()
    dash.training_result_per_epoch(plot_acc=True, plot_loss=True)
    pred_Labels, pred_proba = dash.predict_model()
    # dash.build_confusion_matrix()

    result = model_metrics(testLabelsIndexes, pred_Labels, pred_proba, N, nLabels, typeNamesList,
                           testTypeNames, snTypes)

    result.confusion_matrix(aggregated_confusion_matrix=True)
    ...
    # #try:
    #calc_model_metrics(saveFilename, testLabelsIndexes, testImages, testTypeNames, typeNamesList, snTypes=snTypes)
    # except Exception as e:
    #     print(e)
    #
