...
class AstroDASH(object):
    ...
    def build_model(self):
        ...
        self.model.add(Conv1D(64, 5, activation="relu", input_shape=(self.N,1)))
        ...
        self.history = self.model.fit(reshaped_trainImages, self.trainLabels, validation_split= 0.3,
                                      batch_size=100,epochs=5, verbose=0)
        acc = self.model.evaluate(reshaped_trainImages, self.trainLabels)
        print("Loss:", acc[0], " Accuracy:", acc[1])
        print(self.N, self.nLabels) # self.N = 1024 and self.nLabels = 51




        # # self.model.add(Flatten())
        # self.model.add(Bidirectional(LSTM(32, return_sequences=True, dropout=0.35), input_shape=(self.N,1)))
        # # self.model.add(GRU(32, return_sequences=True, dropout=0.30, input_shape=(self.N,1)))
        # self.model.add(Dropout(0.30))
        # self.model.add(Conv1D(32, 5, activation="relu"))
        # #self.model.add(AveragePooling1D(pool_size=2))
        # self.model.add(MaxPooling1D(pool_size=2))
        # # self.model.add(GRU(32, return_sequences=False, dropout=0.30))
        # # self.model.add(Dropout(0.30))
        # self.model.add(Bidirectional(LSTM(32, return_sequences=False, dropout=0.25)))
        # #self.model.add(Dropout(0.20))
        # self.model.add(Dense(self.nLabels, activation='softmax'))
        # self.model.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        # self.model.summary()
        # self.history = self.model.fit(reshaped_trainImages, self.trainLabels, validation_split= 0.3,
        #                               batch_size=200,epochs=10, verbose=0)
        # acc = self.model.evaluate(reshaped_trainImages, self.trainLabels)
        # print("Loss:", acc[0], " Accuracy:", acc[1])
        # print(self.N, self.nLabels)
