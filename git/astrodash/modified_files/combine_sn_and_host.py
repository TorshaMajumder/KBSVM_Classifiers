...
from preprocessing_tools import redshift_spectrum, mean_zero
...
class CombineSnAndHost(object):
    def __init__(self, snInfo, galInfo, w0, w1, nw):
        ...
        self.preProcess = PreProcessSpectrum(w0=w0, w1=w1, nw=nw)
    ...
    def template_data(self, snCoeff, galCoeff, z):
        ...
        newFlux, continuum = self.preProcess.continuum_removal(binnedWave, binnedFlux, minIndex, maxIndex,self.numSplinePoints)
        meanZero = mean_zero(newFlux, minIndex, maxIndex)
...
class BinTemplate(object):
    def __init__(self, filename, templateType, w0, w1, nw):
        ...
        self.preProcess = PreProcessSpectrum(filename=self.filename,w0=w0, w1=w1, nw=nw)
        self.readSpectrumFile = ReadSpectrumFile(filename=self.filename, w0=w0, w1=w1, nw=nw)
        self.spectrum = self.readSpectrumFile.read_file_type(template=True)
        if templateType == 'sn':
    ...
    def _bin_sn_template(self, ageIdx):
        ...
        if self.snidTemplate:
            ...
            binnedWave, binnedFlux, minIndex, maxIndex = self.preProcess.log_wavelength(wave, flux)
            contRemovedFlux, continuum = self.preProcess.continuum_removal(binnedWave, binnedFlux, minIndex, maxIndex, self.numSplinePoints)
            meanzero = mean_zero(contRemovedFlux, minIndex, maxIndex)
            apodized = self.preProcess.apodize(meanzero, minIndex, maxIndex)
    ...
    def _bin_gal_template(self):
        wave, flux = self.readSpectrumFile.read_two_col_ftype()
        # flux = normalise_spectrum(flux)
        binnedWave, binnedFlux, minIndex, maxIndex = self.preProcess.log_wavelength(wave, flux)
        contRemovedFlux, continuum = self.preProcess.continuum_removal(binnedWave, binnedFlux, minIndex, maxIndex, self.numSplinePoints)
